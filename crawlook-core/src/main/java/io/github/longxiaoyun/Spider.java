...
import io.github.longxiaoyun.downloader.HttpClientDownloader;
...
import io.github.longxiaoyun.pipeline.ResultItemsCollectorPipeline;
import io.github.longxiaoyun.processor.PageProcessor;
import io.github.longxiaoyun.scheduler.QueueScheduler;
import io.github.longxiaoyun.scheduler.Scheduler;
import io.github.longxiaoyun.thread.CountableThreadPool;
import io.github.longxiaoyun.utils.UrlUtils;
import io.github.longxiaoyun.utils.WMCollections;
...
public class Spider implements Runnable, Task {
    ...
    protected void onClose(Spider spider) {
        if (CollectionUtils.isNotEmpty(spiderListeners)) {
            for (SpiderListener spiderListener : spiderListeners) {
                spiderListener.onClose(spider);
            }
        }
    }

    protected void onStart(Spider spider) {
        if (CollectionUtils.isNotEmpty(spiderListeners)) {
            for (SpiderListener spiderListener : spiderListeners) {
                spiderListener.onStart(spider);
            }
        }
    }
    ...
    public void stop() {
        if (stat.compareAndSet(STAT_RUNNING, STAT_STOPPED)) {
            ...
            logger.info("Spider {} stop fail!", getUUID());
        }
    }
